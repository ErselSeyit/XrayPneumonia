...
class ClassifierInceptionV3 {
  ...
  ClassifierInceptionV3({this.numThreads}) {
    _loadModel().then((_) {
      _isModelLoaded = true;
      if (_areLabelsLoaded) {
  ...
  Future<void> _loadModel() async {
    try {
      ...
      if (inputShape.length != 4 || inputShape[1] != 299 || inputShape[2] != 299 || inputShape[3] != 3) {
        throw Exception('Unexpected input shape: $inputShape. Expected [1, 299, 299, 3].');
      }
  ...
  Future<Category> predict(img.Image image) async {
    ...
    Float32List inputBytes = Float32List(1 * 299 * 299 * 3);
    int pixelIndex = 0;
    ...
    final input = inputBytes.reshape([1, 299, 299, 3]);
  ...
  String get modelName => 'InceptionV3_model_finetuned.tflite';
}
